...
JETTY_XML_FILE="${ARCHIVA_SOURCE_DIR}/archiva/conf/jetty.xml"

cat > "${JETTY_XML_FILE}" <<EOF
<?xml version="1.0"?>
    ...
    <Ref id="RequestLog">
      <Set name="requestLog">
        <New id="RequestLogImpl" class="org.eclipse.jetty.server.NCSARequestLog">
          <Set name="filename"><SystemProperty name="jetty.logs" default="${REQUEST_LOG_DIR}"/>/${ARCHIVA_JETTY_LOGS_PATTERN}</Set>
...
if [[ "${DB_TYPE}" == "mysql" ]]; then
  ...
  <Arg>
      ...
      <Set name="url">jdbc:mysql://${DB_HOST}:${DB_PORT}/${DB_NAME}</Set>
